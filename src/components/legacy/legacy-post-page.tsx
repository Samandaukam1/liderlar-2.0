 ...
 *
 * IKKITA eski manzil bir xil maqolani ochadi, chunki 1.0 da ham shunday
 * bo'lgan: sayt bo'limi `/nomzodlar/<slug>` va Tilda'ning o'z yo'li
 * `/tpost/<slug>`. Ikkalasi ham 200 qaytaradi — bir-biriga ham,
 * `/liderlar/...` ga ham YO'NALTIRILMAYDI: eski havolalar tashqi saytlarda,
 * ijtimoiy tarmoqlarda va qidiruv indeksida qolgan, va ularning aksariyati
 * uchun 2.0 da mos keluvchi yozuv umuman yo'q. Redirect ularni noto'g'ri
 * odamning sahifasiga olib borardi yoki 404 ga aylanardi.
 *
 * Kanonik manzil esa BITTA: `/nomzodlar/<slug>`. Sitemapga faqat u kiradi va
 * `/tpost/` sahifasi canonical'da o'shanga ishora qiladi, aks holda bir maqola
 * qidiruvda o'ziga o'zi raqobat qilardi.
 *
 * Kontent eski emas: sahifa 2.0 ning header/footer'i (root layout),
...
export async function generateLegacyPostMetadata(legacySlug: string): Promise<Metadata> {
...
export async function LegacyPostPage({ legacySlug }: { legacySlug: string }) {
  const post = await getLegacyPostBySlug(legacySlug).catch(() => null);
  if (!post) notFound();
